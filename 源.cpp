...
public:
	...
	void input(float shuru[], int pyl)//输入数组和偏移量
	{
		...
		shuru[12 + pyl] = (float)vir_halfdead22, shuru[13 + pyl] = (float)vir_halfdead23, shuru[14 + pyl] = (float)vir_halfdead24, shuru[15 + pyl] = (float)vir_halfdead33, shuru[16 + pyl] = (float)vir_halfdead34, shuru[17 + pyl] = (float)vir_halfdead44;
...
private:
	int myStatus;
	int *stepmap[225];//更改棋盘大小的话，此处需要更改
	int step;
	player p;
